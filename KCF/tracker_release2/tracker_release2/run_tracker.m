...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	base_path = 'C:\Users\ww\Desktop\visualtracking\sequences\';
	...
	otherwise
    	...
    	video_path_2 = [base_path video '/'];
    	filename = [video_path_2 'groundtruth_rect.txt'];
        gt=dlmread(filename);
    name='shaking';
    position_=positions(:,[2,1])-gt(:,[3,4]) / 2;
    dlmwrite([name '.txt'], position_);
    writerObj=VideoWriter('shaking_KCF.avi');
open(writerObj);
boxes=dlmread([name '.txt']);

for i=1:numel(img_files)
    img=imread([video_path img_files{i}]);
    imshow(img);
    hold on
    initstate=[boxes(i,1) boxes(i,2) target_sz(2) target_sz(1)];
