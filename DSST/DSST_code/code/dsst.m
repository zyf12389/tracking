...
[rs, cs] = ndgrid((1:sz(1)) - floor(sz(1)/2), (1:sz(2)) - floor(sz(2)/2));%计算各个patch与模板的中心距，rs相当于gaussian的x
y = exp(-0.5 * (((rs.^2 + cs.^2) / output_sigma^2)));%赋值score
yf = single(fft2(y));%单精度浮点型
...
% scale factors
ss = 1:nScales;
scaleFactors = scale_step.^(ceil(nScales/2) - ss);%尺度倍数
